    ...
    <div class="user-profile-container">
      ...
      <div class="user-info-card">
        <div class="user-header">
          ...
          <div class="user-details">
            ...
            <p v-if="user.status === 1 || user.status === '1'" class="user-status active">
      ...
      <div class="user-posts-section">
        ...
        <div v-if="totalPages > 1" class="pagination-container">
          <button class="btn page-btn" :class="{ disabled: currentPage === 1 }" @click="changePage(currentPage - 1)">
          ...
          <button class="btn page-btn" :class="{ disabled: currentPage === totalPages }"
...
export default {
  ...
  setup(props) {
    ...
    const filteredPosts = computed(() => {
      ...
      return posts.value.filter(post => post.postCategoryName === selectedCategory.value);
    });
    ...
    const followUser = async () => {
      try {
        const response = await toggleFollow(user.id, true, store.state.user.id);
        if (response.success === true) {
          user.is_followed = true;
          userStats.followerCount++;
          UToast({ message: '关注成功！', type: 'success', duration: '3500' })
    ...
    const unfollowUser = async () => {
      try {
        const response = await toggleFollow(user.id, false, store.state.user.id);
        if (response.success === true) {
          user.is_followed = false;
          userStats.followerCount--;
          UToast({ message: '取关成功！', type: 'success', duration: '3500' })
          // 如果项目中有消息提示组件，可以使用它来显示成功消息
        } else {
